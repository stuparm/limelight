// Command example is a service shaped like a real one: a gin REST API on its
// own port, and limelight's control endpoint on a separate admin listener that
// production would not route from outside.
//
